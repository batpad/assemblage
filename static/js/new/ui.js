CUi = {
	...
	setTouchCB: function( sCB, mCB, eCB )
	{
		CUi.doc.addEventListener('touchstart', sCB );
		CUi.doc.addEventListener('touchmove', mCB );
		CUi.doc.addEventListener('touchend', eCB );
	},


	_m_oldMoseDown: false,
	_m_oldMoseMove: false,
	_m_oldMoseUp: false,
	
	reSetMouseRawCB: function( sCB, mCB, eCB )
	{
		if( CUi._m_oldMoseDown)
		{
			CUi.doc.onmousedown = CUi._m_oldMoseDown;
			CUi.doc.onmousemove = CUi._m_oldMoseMove;
			CUi.doc.onmouseup = CUi._m_oldMoseUp;
		}
	},
	
	setMouseRawCB: function( sCB, mCB, eCB )
	{
		CUi._m_oldMoseDown = CUi.doc.onmousedown;
		CUi._m_oldMoseMove = CUi.doc.onmousemove;
		CUi._m_oldMoseUp = CUi.doc.onmouseup;
		
		CUi.doc.onmousedown = sCB;
		CUi.doc.onmousemove = function(e) { /*CUi.setMouseXY(e); */ mCB(e); }
		CUi.doc.onmouseup = eCB;
	},
	
	logFB: function(msg)
	{
		if(console.debug) { console.debug(CUtil.getNow() + ": " + msg); }
		else if(console.log) { console.log(CUtil.getNow() + ": " + msg); }
	},
	
	_M_AGENT_FF: "Firefox",
	_M_AGENT_CH: "Chrome",
	_M_AGENT_IE: "MSIE",
	_M_AGENT_ANDROID: "Android",
	_M_AGENT_SF: "Safari",
	_M_AGENT_SF_VVDET: "Version",
	
	browserDetect: function()
	{
		CUi._isCH = false; CUi._isIE = false; CUi._isFF = false; CUi._isSF = false; CUi._isAndroid = false;
				
		if( ( chi = navigator.userAgent.indexOf(CUi._M_AGENT_ANDROID) ) !=-1 )
		{		
			CUi._isAndroid = true;
		}
		else if( ( chi = navigator.userAgent.indexOf(CUi._M_AGENT_CH) ) !=-1 )
		{
	...
	get_view_cxy: function(doW)
	{
		var winW = 630, winH = 460;
		
		try {
			if (CUi.doc.body && CUi.doc.body.offsetWidth) {
				winW = CUi.doc.body.offsetWidth;
				winH = CUi.doc.body.offsetHeight;
			}
			
			if (CUi.doc.compatMode=='CSS1Compat' &&
				CUi.doc.documentElement &&
				CUi.doc.documentElement.offsetWidth ) {
					winW = CUi.doc.documentElement.offsetWidth;
					winH = CUi.doc.documentElement.offsetHeight;
			}
	...
	{
		var el = CUi.createAbsElm('div',CUi._M_WORKING_EID);
		
		el.innerHTML = "Saving Please Wait ...";
		el.style.backgroundColor = '#FFF988';
		el.style.color='#000';
		...
		CUtil.capture( CUi._M_WORKING_EID, function(el) {
			el.style.left = ((CUtil.getDim(true, 0) / 2) - (el.offsetWidth / 2))  + 'px';
			el.style.top = '300px';
		} );
	},
