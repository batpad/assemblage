...
CUtil = {
	...
	// function is commented because its conflicting with the fabric.js
	/*
	...
	{		
		if((ob.getAttribute)&&(ob.getAttribute("name")=="no-selection"))
			...
			return(true); //HACK :: as fabric is misinterpreting the mouse move & gives error in console
	},
	...
	isPointInBound: function(x,y,bound)
	{
		return( (x >= bound[0]) && (y >= bound[1]) && (x <= bound[3]) && (y <= bound[4]) );
	},
	
	isPointInRect: function(r,p)
	{									   
		return( (p[0] >= r[0] && p[0] <= r[2]) && (p[1] >= r[1] && p[1] <= r[3]) );
	},
	
	isPointInRectWH: function(r,p)
	{
		r[2] += r[0]; r[3] += r[1]; 
		//CUi.logFB( ' isPointInRectWH = ' + p + " - " + r);
		return( (p[0] >= r[0] && p[0] <= r[2]) && (p[1] >= r[1] && p[1] <= r[3]) );
	},
	...
	findPos: function(obj) {
		...
		return [curleft,curtop,(curleft + actualOBJ.offsetWidth),(curtop + actualOBJ.offsetHeight)];
	},
	
	ldist: function( point1, point2 )
    {
		var xs = 0;
		var ys = 0;
	...
	findPosRelativeToViewport: function(obj) {
		var objPos = CUtil.findPos( obj );
		var scroll = CUtil.getPageScroll();
		return(objPos);
	},
	...
	getObjectDimOff: function(o)
	{
		return( [o.offsetLeft,o.offsetTop,o.offsetLeft+o.offsetWidth,o.offsetTop+o.offsetHeight] );
	},
